---
name: exmail-io
description: 收发邮件：发信（纯文本/HTML/附件/抄送密送/内联图片/回复转发）、查收件箱、搜索、读正文、下载附件、标记已读、修改邮件日期。当用户要发邮件、随便发一封邮件、给某个地址发封邮件、看有没有新邮件、读或回复某封邮件时使用；即使没提 IMAP/SMTP 或企业邮箱也要用，不要自己写 smtplib/imaplib 脚本。发信账号需托管在腾讯企业邮箱（exmail，自有企业域名同样适用），收件人域名不限。
---
